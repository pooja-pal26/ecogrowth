...
* File Description  : IndexController
* Created By : Puneet Mishra
* Created Date: 26 March 2017
 */
...
//		/*************** check user identity ************/
    ...
    */	
    ...
    public function myProfileAction(){
        ...
        $this->view->messages 		= $this->_flashMessenger->getMessages();
        $this->view->getUserRecord = $getAdminRecord= $admin->getUserRecord($this->id);       
   	}
    ...
    */	
    ...
    public function editProfileAction(){
        ...
        $this->view->params 		= $params;
        $this->view->messages 		= $this->_flashMessenger->getMessages();
        $this->view->getUserRecord = $getUserRecord= $admin->getUserRecord($this->id);

        if($this->getRequest()->isPost()){
        	if($params['oldPassword']!='' && sha1(md5($params['oldPassword']))!=$getUserRecord['password']){
				...
				$db->update('tbl_user',$data,array('id=?'=>$params['id']));
				$this->_flashMessenger->addMessage('User Records updated successfully');          	                           
				$this->_redirect('/index/my-profile');
        	} 
        }        
   	}
		...
		if($this->getRequest()->isPost()){
			if($form->isValid($_POST)){
				...
				if($result){
					...
					$msg ='<div style="margin: 0% auto; width: 535px; border:1px solid #e4e2e2; padding:7px;">';
					$msg  .= '<div style=" background: url("'.$this->siteurl.'img/login-background.png");border: 1px solid rgba(54, 64, 74, 0.05);border-radius: 5px;margin-bottom: 20px;padding: 5px; ">';
					$msg .= '<div style="border-bottom: 1px solid #254AA5"> ';
					$msg .= '<img  src="'.$this->siteurl.'img/dashboard-logo.png" style="width: 12%; height:67px;"><img  src="'.$this->siteurl.'img/sterlite-logo.png" style="width: 26%; margin-left: 3px; margin-bottom: -10px;">';
					$msg .= '</div> ';
					$msg .= '<br clear="all">';
					$msg .= '<div style="color: rgb(92, 97, 95);">';		
					$msg .= "Hi <strong>".$StaffName."</strong> reset your password by following link...<br><br>";
					$msg .= "<strong><a href='".$this->siteurl."admin/reset-password/token/".$token."'>Click here for reset your password</a> <br>";
					$msg .= "Thanks,<br>";
					$msg .= "Sterlite Team";	
					$msg .= '<br clear="all" style="margin-bottom: 20px;">';	
					$msg .= '<a href="http://logimetrix.co.in" style="margin-top: 20px; margin-left: 120px; border-top: 1px solid rgb(171, 230, 204);">Powered by Logimetrix Techsolutions Pvt. Ltd.</a>';	
					$msg .= '<br clear="all">';						
					$msg .= '</div>';	
					$msg .= '</div>';
					$msg .= '</div>';
					...
					try {
						...
						$mail->setFrom($from_email, 'Sterlite Power-PMMS');
        ...
        if($result){
					if($this->getRequest()->isPost()){
						if($params['newPassword']!=$params['confirmPassword']){
							...
							$this->_flashMessenger->addMessage('Your Password reset successfully');          	                           
