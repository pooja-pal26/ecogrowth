...
const DebitAccountDetails = () => {
  ...
  const loadData = async () => {
    try {
      const res = await fetchList('dynamic/debitaccountdetailss');
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        await deleteItem('dynamic/debitaccountdetailss', id);
        loadData();
      } catch (error) {
  ...
  const handleSubmit = async () => {
    try {
      if (isEditing) {
        await updateItem('dynamic/debitaccountdetailss', editId, formData);
      } else {
        await createItem('dynamic/debitaccountdetailss', formData);
      }
