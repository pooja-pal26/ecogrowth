...
const NatureOfWork = () => {
  ...
  const loadData = async () => {
    ...
    try {
      let res;
      try {
        res = await fetchList('nature-of-work');
      } catch {
        res = await fetchList('dynamic/natureofworks');
      }
      if (res && res.success) {
        setData(res.data || []);
      }
    } catch (error) {
      console.error('Failed to load nature of work list:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure? Do you want to delete clicked nature of work?')) {
      return;
    }
    ...
    try {
      let res;
      try {
        res = await deleteItem('nature-of-work', id);
      } catch {
        res = await deleteItem('dynamic/natureofworks', id);
      }

      if (res && (res.message || res.title)) {
        alert(res.message || 'Nature of work deleted successfully');
      }
      loadData();
    } catch (error) {
      console.error('Failed to delete nature of work:', error);
      alert(error.response?.data?.message || 'Failed to delete nature of work');
    }
  };
  ...
  const handleEdit = (row) => {
    setFormData({
      nature_of_work: row.nature_of_work || row.name || ''
    });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.nature_of_work.trim()) {
      alert('Nature of Work Missing! Please enter nature of work.');
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        name: formData.nature_of_work.trim()
      };

      if (isEditing) {
        try {
          res = await updateItem('nature-of-work', editId, payload);
        } catch {
          res = await updateItem('dynamic/natureofworks', editId, payload);
        }
      } else {
        try {
          res = await createItem('nature-of-work', payload);
        } catch {
          res = await createItem('dynamic/natureofworks', payload);
        }
      }
      ...
      console.error('Failed to save nature of work:', error);
  ...
  const columns = [
    {
      ...
      render: (row) => row.nature_of_work || row.name || '-'
    }
  ];
  ...
  return (
    <div className="space-y-6">
      <MasterDataTable
        ...
        loading={loading}
        onAdd={handleAdd}
        addButtonText="Add Nature Of Work"
        renderActions={(row) => {
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData((prev) => ({ ...prev, nature_of_work: e.target.value }))
                  }
