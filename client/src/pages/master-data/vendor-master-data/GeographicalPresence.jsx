...
const GeographicalPresence = () => {
  ...
  const loadData = async () => {
    try {
      const res = await fetchList('dynamic/geographicalpresences');
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        await deleteItem('dynamic/geographicalpresences', id);
        loadData();
      } catch (error) {
  ...
  const handleSubmit = async () => {
    try {
      if (isEditing) {
        await updateItem('dynamic/geographicalpresences', editId, formData);
      } else {
        await createItem('dynamic/geographicalpresences', formData);
      }
