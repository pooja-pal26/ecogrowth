...
const ProductBrand = () => {
  ...
  const loadData = async () => {
    ...
    try {
      let res;
      try {
        res = await fetchList('product-brands');
      } catch {
        res = await fetchList('dynamic/productbrands');
      }
      if (res && res.success) {
        setData(res.data || []);
      }
  ...
  const handleDelete = async (row) => {
    ...
    const name = row.brand_name || row.name || 'selected product brand';

    if (!window.confirm(`Do you want to delete ${name} from product brand?`)) {
      return;
    }
    ...
    try {
      let res;
      try {
        res = await deleteItem('product-brands', id);
      } catch {
        res = await deleteItem('dynamic/productbrands', id);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.brand_name.trim()) {
      alert('Product Brand Name Missing! Please enter product brand name.');
      return;
    }
    ...
    try {
      ...
      if (isEditing) {
        try {
          res = await updateItem('product-brands', editId, payload);
        } catch {
          res = await updateItem('dynamic/productbrands', editId, payload);
        }
      } else {
        try {
          res = await createItem('product-brands', payload);
        } catch {
          res = await createItem('dynamic/productbrands', payload);
        }
      }
  ...
  const columns = [
    {
      ...
      render: (row) => row.brand_name || row.name || '-'
    }
  ];
  ...
  return (
    <div className="space-y-6">
      <MasterDataTable
        ...
        loading={loading}
        onAdd={handleAdd}
        addButtonText="Add Product Brand"
        renderActions={(row) => {
          return (
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200 dark:border-slate-800">
            ...
            >
              <h3 className="text-lg font-bold">
                {isEditing ? 'Edit Product Brand' : 'Add Product Brand'}
              </h3>
              <button
