...
const StockOutReport = () => {
  ...
  const handleExportCSV = () => {
    const headers = ['#', 'Stock Out Date', 'Allocated To', 'Allocated By', 'PO Number', 'Site ID', 'Remarks'];
    ...
    <div className="px-2 pt-1 pb-3 sm:px-4 sm:pt-1 sm:pb-4 w-full max-w-7xl mx-auto space-y-3 font-sans">
      ...
      >
        <h1 className="text-base font-bold tracking-tight text-white flex items-center gap-2">
          <span>Material Stock Out Report</span>
        </h1>
      ...
      <div className="bg-white rounded-b-lg border-x border-b border-gray-200 shadow-sm overflow-hidden p-3 sm:p-4 space-y-3">
        ...
        <div className="bg-white p-3 rounded-lg border border-gray-200 shadow-xs">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 items-end">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1">Site ID</label>
              <select
                value={selectedSite}
          ...
          <table className="w-full text-left text-xs sm:text-sm text-gray-700">
            ...
            >
              <tr>
                ...
                <th className="px-3 py-2.5">Stock Out Date</th>
                <th className="px-3 py-2.5">Allocated To</th>
                    ...
                    <tr key={row.id} className="hover:bg-teal-50/40 transition-colors">
                      ...
                      <td className="px-3 py-2 text-center whitespace-nowrap">
                        <div className="flex items-center justify-center space-x-2">
                          ...
                          <Link
                            to={`/material-stock/stock-out?id=${row.id}`}
      ...
      {/* Stock Out Details Modal matching PHP view-stock-out.phtml */}
      {isModalOpen && (
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 p-3 bg-gray-50 rounded border border-gray-200 text-xs text-gray-700">
                    <div>
                      ...
                      <span className="font-semibold text-gray-900">{selectedStockOut.stock_out_date || '-'}</span>
                    </div>
                    <div>
                      <span className="text-gray-500 block">Allocated To:</span>
                      <span className="font-semibold text-gray-900">{selectedStockOut.received_by || '-'}</span>
                    </div>
                    <div>
                      <span className="text-gray-500 block">Allocated By:</span>
                      <span className="font-semibold text-gray-900">{selectedStockOut.allocated_by || '-'}</span>
                    </div>
                    <div>
                      <span className="text-gray-500 block">PO Number:</span>
                      <span className="font-semibold text-gray-900">{selectedStockOut.po_no || '-'}</span>
                    </div>
                    <div>
                      <span className="text-gray-500 block">Site ID:</span>
                      <span className="font-semibold text-gray-900">{selectedStockOut.site_id || '-'}</span>
                    </div>
                    <div className="col-span-3">
                      <span className="text-gray-500 block">Remarks:</span>
                      <span className="font-semibold text-gray-900">{selectedStockOut.remarks || 'None'}</span>
                    </div>
                  </div>
                    ...
                    <table className="w-full text-left text-xs">
                      ...
                      <tbody className="divide-y divide-gray-200 bg-white">
                        {selectedStockOut.items && selectedStockOut.items.length > 0 ? (
                          ...
                          <tr>
                            <td colSpan="5" className="py-4 text-center text-gray-400">No items recorded in this stock out.</td>
                          </tr>
                        )}
                      </tbody>
                      <tfoot className="bg-gray-50 font-semibold text-gray-800 border-t border-gray-200">
                        <tr>
                          ...
                          <td className="px-3 py-2 text-right text-teal-700 font-bold">{selectedStockOut.total_quantity}</td>
                        </tr>
                      </tfoot>
                    </table>
                  </div>
